...
\multicolumn{1}{c}{} & & & \multicolumn{1}{c}{BMSSP} & BMSSP-CD & \\ \hline"""
    ...
    for index, row in df.iterrows():
        ...
        bmssp_CD_tempo = formatar_erro(row['Tempo BMSSP-CD (ms)'], row['Desvio BMSSP-CD (ms)'])

        n_str = row['Número de Vértices']
        m_str = row['Número de Arestas']
        
        ratio_str = f"{row['Ratio BMSSP-CD / BMSSP']:.3f}"
    ...
    if parsed_results:
        ...
        if df.empty:
            ...
            df_summary.rename(columns={
                ...
                'bmssp-CD_time': 'Tempo BMSSP-CD (ms)',
                'bmssp_time': 'Tempo BMSSP (ms)',
                'bmssp-CD_std': 'Desvio BMSSP-CD (ms)',
                'bmssp_std': 'Desvio BMSSP (ms)',
                'n': 'Número de Vértices',
                'm': 'Número de Arestas'
            }, inplace=True)

            df_summary['Ratio BMSSP-CD / BMSSP'] = df_summary.get('Tempo BMSSP-CD (ms)', np.nan) / df_summary.get('Tempo BMSSP (ms)', np.nan)
