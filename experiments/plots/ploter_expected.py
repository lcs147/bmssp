...
def create_and_save_ratio_plot(df_ratio, filename, output_dir):
    ...
    try:
        ...
        ax.plot(
            ...
            linestyle='-',
            color='r',
            label='BMSSP Time / Dijkstra Time'
        )
        ...
        ax.set_ylabel('Performance Ratio (BMSSP Time / Dijkstra Time)')
        ax.legend()
        ax.grid(True, which="both", ls="--", linewidth=0.5)
...
def save_performance_table(df, filename, output_dir):
    ...
    try:
        ...
        for col in ['Tempo Dijkstra (ms)', 'Tempo BMSSP (ms)','Desvio Dijkstra (ms)','Desvio BMSSP (ms)', 'Ratio BMSSP / Dijkstra']:
...
\multicolumn{1}{c}{} & & & \multicolumn{1}{c}{BMSSP} & BMSSP-Expected & \\ \hline"""
    ...
    for index, row in df.iterrows():
        bmssp_tempo = formatar_erro(row['Tempo BMSSP (ms)'], row['Desvio BMSSP (ms)'])
        bmssp_CD_tempo = formatar_erro(row['Tempo BMSSP-Expected (ms)'], row['Desvio BMSSP-Expected (ms)'])

        n_str = row['Número de Vértices']
        m_str = row['Número de Arestas']
        
        ratio_str = f"{row['Ratio BMSSP-Expected / BMSSP']:.3f}"
        linha = (f"{row['Graph File']} & {n_str} & {m_str} & {bmssp_tempo} & {bmssp_CD_tempo} & {ratio_str} \\\\")
...
if __name__ == "__main__":
    ...
    if parsed_results:
        ...
        if df.empty:
            ...
            if 'bmssp-wc' in df_pivot.columns and 'bmssp-expected' in df_pivot.columns:
                df_pivot['ratio'] = df_pivot['bmssp-expected'] / df_pivot['bmssp-wc']
            ...
            df_summary.rename(columns={
                ...
                'bmssp-expected_time': 'Tempo BMSSP-Expected (ms)',
                'bmssp-wc_time': 'Tempo BMSSP (ms)',
                'bmssp-expected_std': 'Desvio BMSSP-Expected (ms)',
                'bmssp-wc_std': 'Desvio BMSSP (ms)',
                'n': 'Número de Vértices',
                'm': 'Número de Arestas'
            }, inplace=True)

            df_summary['Ratio BMSSP-Expected / BMSSP'] = df_summary.get('Tempo BMSSP-Expected (ms)', np.nan) / df_summary.get('Tempo BMSSP (ms)', np.nan)
            ...
            cols_order = [
                ...
                'Tempo BMSSP (ms)',
                'Desvio BMSSP (ms)',
                'Tempo BMSSP-Expected (ms)',
                'Desvio BMSSP-Expected (ms)',
